# -*- coding: utf-8 -*-
"""suc_smolagents_ollama_searchweb_With_improvements.ipynb
...
agent = CodeAgent(
    ...
    model=model,
    add_base_tools=True,
    additional_authorized_imports=["markdown", "json", "csv"]  # إضافة مكتبات إضافية
)

""" نسخة محسنة مع أدوات إضافية"""

from smolagents import CodeAgent, LiteLLMModel, DuckDuckGoSearchTool, tool

# أداة مخصصة لتنظيف النتائج
@tool
def clean_search_results(results: str) -> str:
    """
    Clean and format search results for better readability.

    Args:
        results (str): Raw search results to clean.

    Returns:
        str: Cleaned and formatted search results.
    """
...
agent = CodeAgent(
    tools=[DuckDuckGoSearchTool(), clean_search_results],
    model=model,
    add_base_tools=True,
    additional_authorized_imports=["markdown", "json"]
)

# تشغيل الوكيل
print("=== Enhanced Search Test ===")
result = agent.run("""
Search for the latest AI news, clean the results, and provide a structured summary
with sections for: Technology Updates, Company News, and Ethical Considerations
""")
print(f"\n=== Enhanced Result ===\n{result}")

"""نسخة مع تحكم أفضل في العملية:"""
...
model = LiteLLMModel(
    ...
    api_base="http://127.0.0.1:11434",
    num_ctx=8192,
    temperature=0.3,  # تقليل العشوائية للنتائج أكثر اتساقاً
)
...
agent = CodeAgent(
    ...
    model=model,
    add_base_tools=True,
    max_steps=6,  # تحديد أقصى عدد من الخطوات
    additional_authorized_imports=["markdown", "json"]
)

# مهمة أكثر تحديداً
print("=== Structured Search Test ===")
result = agent.run("""
Search for the latest AI news and provide a structured response with:
1. Top 5 AI developments this week
2. Major companies mentioned in the news
3. Any controversial AI topics
4. Future predictions mentioned in the articles
Use markdown formatting for the response.
""")
print(f"\n=== Structured Result ===\n{result}")

""" نسخة مع معالجة متعددة اللغات:"""
...
# مهمة باللغة العربية
print("=== Arabic Search Test ===")
result = agent.run("""
ابحث عن أحدث أخبار الذكاء الاصطناعي باللغة العربية وقدم ملخصاً شاملاً
يركز على التطورات التقنية والشركات الرائدة في هذا المجال
""")
print(f"\n=== Arabic Result ===\n{result}")



!ollama run hf.co/bartowski/Llama-3.3-70B-Instruct-abliterated-GGUF:IQ1_M
...
model = LiteLLMModel(
    model_id="ollama_chat/hf.co/bartowski/Llama-3.3-70B-Instruct-abliterated-GGUF:IQ1_M",
    api_base="http://127.0.0.1:11434",
    num_ctx=8192,
)
...
print("=== Arabic Search Test ===")
result = agent.run("""
Find the Internet Archive website and search for Reader's Digest and download any issue pdf.
""")
print(f"\n=== Arabic Result ===\n{result}")
